...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <h3 class="clr">CHI TIẾT GIAO DỊCH</h3>
      ...
      @foreach($cthd as $ct)
              <form class="form-inline" role="form" method="post" enctype="multipart/form-data" action="e-stu/thanhtoan/postcheckout/{{$ct->mahd}}">
                      @csrf
    <div>
        <h4 style="color:#6a7382;text-align:center;"><strong>HÓA ĐƠN THÁNG {{date('m/Y',strtotime($ct->denngay))}}</strong></h4>
    </div>
      <div class="col-md-12">
     ...
     <div>
        <p style="text-align:center"><label><input name="submit" type="submit" value="Thanh toán" class="btn btn-round btn-info hidden-print"></label></p>
    </div>
  </div>
  @endforeach
                {{csrf_field()}}
